...
- =org-journal-file-header= - a string that will be inserted at the top of
  every new journal file. It can also be a function, the return value will
  than be inserted. The time of the new journal entry will be passed to that
  function, so you can use it the pass it `format-time-string` for example.
